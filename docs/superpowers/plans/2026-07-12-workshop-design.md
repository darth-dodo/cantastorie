...
- The six run states are the complete vocabulary; `records.py`'s transition table is law — reject is `staged → rejected`, delete is record removal (any settled state), run-again is a **fresh run** (new record, same PackRequest), never a state change on the old one.
- Deleting an **approved** run unpublishes its stories first (prototype footer: "deleting an approved story unpublishes it from the shelf"): remove the story from the language manifest and delete `published/stories/{id}/…` objects. `publish.py` stays the only module touching `published/`.
...
### Task 1: Backend lifecycle — reject, delete (with unpublish), run-again

**Files:**
- Modify: `src/workshop/records.py` (`RunStore.delete`)
- Modify: `src/pipeline/publish.py` (`unpublish_story`)
- Modify: `src/api/routes/workshop.py` (three POST routes)
- Test: `tests/workshop/test_records.py`, `tests/pipeline/test_publish.py` (or wherever publish specs live), `tests/workshop/test_routes.py`

**Interfaces:**
- `RunStore.delete(self, family_token: str, run_id: str) -> None` — delete the record object; missing object is a no-op (idempotent, mirrors `_MISSING_CODES` handling in `load`).
- `unpublish_story(story_id: str, settings: Settings, *, client: S3Client | None = None) -> None` in `src/pipeline/publish.py` — the mirror of `publish_story`: load the story's language manifest, drop the story's entry, rewrite the manifest, delete every object under `published/stories/{story_id}/`. Missing manifest or story entry is a no-op. Match `publish_story`'s signature/style (look at it first).
- Routes (all require session auth like existing writes; unauthenticated → redirect to login):
  - `POST /workshop/runs/{run_id}/reject` — `record.advance("rejected")`, save, redirect 303 to `/workshop`.
  - `POST /workshop/runs/{run_id}/delete` — if record state is `approved`, unpublish each of `record.story_ids` first; then `store.delete`, redirect 303 to `/workshop`. Deleting a `queued`/`running` run is rejected with 409 (the manager may be mid-flight).
  - `POST /workshop/runs/{run_id}/again` — build a fresh run via `manager.submit(OPERATOR_TOKEN, record.request)`, schedule `manager.execute` as a background task (same as `start_run`), redirect 303 to the **new** run's page.

- [ ] Step 1: failing tests — store delete (save→delete→load is None; delete of missing id no-op), unpublish (published story disappears from manifest + objects gone; second call no-op), route specs (reject settles staged run; delete removes approved run and unpublishes its stories — assert via the harness's published list or bucket contents; delete of a running run → 409; again creates a new queued/staged record with the same request and redirects to its page; every route 404s/redirects without auth). Follow the existing `_Harness` pattern in `tests/workshop/test_routes.py`.
- [ ] Step 2: implement until green. `uv run pytest tests/workshop tests/pipeline` passes; ruff + mypy clean.
- [ ] Step 3: commit.
