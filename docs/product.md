...
> **Narrator voice at launch (honest caveat).** The **One voice** goal is the target, not yet a proven result. Launch narration is generated by **Voxtral** (via OpenRouter), which uses a **separate voice per language** — so a single identity across all five languages is still an open question, with zero-shot voice cloning under evaluation as the path to it. **ElevenLabs** (one multilingual voice, warmer synthesis) is being evaluated in parallel for bedtime warmth. The choice, cost, and trade-offs are recorded in [ADR-002](adr/ADR-002-narration-provider.md); warmth and cross-language consistency are validated on the first Italian story before the library is built.
...
> **How prompts are narrated (launch caveat).** Prompt audio is generated by the same narration provider as story pages — **Voxtral via OpenRouter** to start (per-language voices; **ElevenLabs** under evaluation for warmth). Greek narration support is **unverified** and confirmed before any Greek audio is produced. Separately, **reading-mode timing capture is deferred**: the starting provider returns audio without word timings, so karaoke highlighting's per-word sync is reconstructed later (at reading mode, slice 6) rather than captured at narration time. See [ADR-002](adr/ADR-002-narration-provider.md).
...
- **Factory**: a plain-Python authoring pipeline (Pydantic AI over OpenRouter for stories, safety verdicts, glosses, images, and narration — narration on Voxtral via OpenRouter to start, per-language voices and no word timings yet; ElevenLabs deferred and under evaluation, see [ADR-002](adr/ADR-002-narration-provider.md)) — a local CLI in Phase 1, the same functions behind routes in Phase 2
...
- [Architecture Decision Records](adr/) — Settled decisions, including [ADR-002](adr/ADR-002-narration-provider.md) (narration provider)
