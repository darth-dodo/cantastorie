...
> **Narrator voice at launch (honest caveat).** The **One voice** goal is the target, not yet a proven result. Launch narration is generated by **Gemini TTS** (via OpenRouter): **one house voice** is selected from its roster and pinned across every language, with cross-language consistency an explicit bake-off criterion — its warmth and its Greek are still unproven until that test. Family voice cloning runs separately on **Voxtral via the Mistral API**. **ElevenLabs is retired**; the fallback voice bench is **Deepgram Aura** (`it`/`es`/`de`), and word timings come from a Deepgram transcription pass. The choices, costs, and trade-offs are recorded in [ADR-004](adr/ADR-004-narration-deepgram-voxtral.md) and [ADR-008](adr/ADR-008-narration-gemini-defaults-mistral-cloning.md); warmth and cross-language consistency are validated on the first Italian story (Gemini's roster vs the Aura bench) before the library is built.
...
> **How prompts are narrated (launch caveat).** Prompt audio is generated by the same narration provider as story pages — **Gemini TTS via OpenRouter** (one pinned house voice; **Deepgram Aura** presets are the fallback bench for warmth, ElevenLabs retired). Greek narration support is **unverified**, gated on a listening test before any Greek audio is produced (MAI-Voice-2 is the named fallback; failing both, the Greek shelf launches later rather than worse). Separately, **reading-mode timing capture is deferred**: the narrator returns audio without word timings, so karaoke highlighting's per-word sync is reconstructed later (at reading mode, slice 6) via a **Deepgram transcription pass** rather than captured at narration time. See [ADR-004](adr/ADR-004-narration-deepgram-voxtral.md) and [ADR-008](adr/ADR-008-narration-gemini-defaults-mistral-cloning.md).
...
- [Architecture Decision Records](adr/) — Settled decisions, including [ADR-004](adr/ADR-004-narration-deepgram-voxtral.md) and [ADR-008](adr/ADR-008-narration-gemini-defaults-mistral-cloning.md) (narration providers)
